...
def _get_all_sub_files(pth: Path):
    # rglob("*") oes through all subdirectories and lists all files and directories
...
class SpawnLoader(BaseLoader):
    ...
    def get_source(
        ...
        sub_files = _get_all_sub_files(self.searchpath)
...
def move_other_files(template_path: Path, target_path: Path, prefix_name: str = set_prefix_name):
    """Move all files that are not template files to the other directory.

    The directory must exist.
    """
    for file_pth in _get_all_sub_files(template_path):
...
def spawn_write(config_path: Path, template_path: Path, target_path: Path, prefix_name: str = set_prefix_name):
    ...
    move_other_files(template_path, target_path, prefix_name)
